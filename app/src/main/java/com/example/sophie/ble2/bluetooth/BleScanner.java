...
//start scanning for devices
    public void startScanning(final ScanResultsConsumer scan_results_consumer, long stop_after_ms) {
        ...
        ParcelUuid parcelUuid = new ParcelUuid(UUID.fromString(monitored_device));
...
//count average
            ...
            if (enough_data) {
                ...
                if (distance < final_setting) {
                    ...
                    toast.show();
                    // beep
                    ToneGenerator toneGen1 = new ToneGenerator(AudioManager.STREAM_MUSIC, 15);
...
//scanning or not?
    ...
    private double loaded_final(){
        ...
        SharedPreferences sp = context.getApplicationContext().getSharedPreferences(monitored_device, MODE_PRIVATE);
        return Double.longBitsToDouble(sp.getLong("final_result", Double.doubleToLongBits(2.0)));
    }
// end of class
}
